...
\begin{figure}
  ...
  \caption[Block group 3]{US Census block group 3}
  \label{bg3_map}
\end{figure}
...
Table \ref{tab:RW_HH} shows the number of households in
Radnor-Winston, as well as a breakdown by occupancy. As can be seen in
this table, Radnor-Winston has a higher occupancy rate than our
surrounding census tract, as well as the city of Baltimore. It's
comparable to the state of Maryland overall.
...
Breakdown of races in RW

Table \ref{tab:rw_race_2} shows the self-identified racial identification
of the people in the Radnor-Winston neighborhood.

<<race_tab, echo=FALSE, include=FALSE>>=
...
rw_race_sum$label <- rw_race_sum$label %>%
    ...
    str_replace_all("[^!]*!!", "-") #Replace each !!.* with dash
@
<<rw_race_2, echo=FALSE, position="h">>=

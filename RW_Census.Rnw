...
The data in table \ref{tab:SexbyAge2} can be represented graphicly in
a ``population pyramid.'' Figure \ref{PopPyr2} shows the population
...
is so small overall (about ??? people), the number of individuals in
each age range is equally small, leading to a large statistical
variability in the population percentages. For instance, the largest
age group for females is 70-74 years, which is probably just a
statistical abnormallity rather than a statistically significant
finding.
